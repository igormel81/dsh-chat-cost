...
После этого перезапустите хост: композиция профиля собирается при загрузке, поэтому работающий хост остаётся на той версии, с которой стартовал. Узнать, какой релиз работает, можно и без терминала — наведите на виджет, подсказка называет версию, и каждая запись лога несёт её в поле `plugin` (`"plugin":"dsh-chat-cost@0.6.6"`), поэтому запись можно проследить до выпустившего её релиза.
