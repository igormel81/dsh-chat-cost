...
- **Лог расхода в папке проекта.** `<проект>/.dsh-cost/cost.jsonl` — по одному объекту JSON на сессию за проход, с положением в дереве, четырьмя корзинами токенов, кумулятивной и дельта-стоимостью и источником цены. При первой записи `.dsh-cost/` добавляется в `.gitignore` проекта.
