...
В день с активным поиском эти вызовы могут составлять большую часть счёта. Проверка по выгрузке платформы (24 авг — 22 сен 2026): в дни без поиска сессионная цифра и цифра платформы сходились в пределах $0.04, а в дни с сотнями поисков разница шла за их числом — примерно $0.004 за поиск.

`npm run reconcile` — это и есть та проверка, и её можно повторить:

```sh
node scripts/reconcile.mjs --export ~/Downloads/usage_data_2026-08-24_2026-09-22.zip
```

Скрипт читает выгрузку платформы и логи сессий этого хоста, сворачивает логи тем же правилом «замена, а не сложение», которым пользуется собственная проекция расхода харнесса, считает разницу по тарифам самой платформы за тот день и печатает её рядом с числом веб-поисков за день. И прежде чем что-то сообщить, он сверяет свёртку каждой сессии с долговечными счётчиками харнесса и отказывается обвинять платформу, когда не сходится его собственная арифметика — самая вероятная ошибка такого инструмента.
