...
import { turnUsage } from './turns.js'
...
async function collectTree(ctx, rootId, signal) {
  ...
  if (query !== undefined && typeof query.traceSession === 'function') {
    try {
      ...
      // The query engine reads the persisted corpus, so it knows where a session
      // lived even when the store no longer holds it live: that path is what lets
      // a chat that is not currently open be priced from the cost log.
      const traced = trace?.target?.header ?? trace?.root?.header ?? null
      const workspace = typeof traced?.cwd === 'string' && traced.cwd !== '' ? traced.cwd : null
      ...
      return { ids: unique([root, ...ids]), parents, complete, workspace }
    } catch {
  ...
  return { ids: unique(ids), parents, complete, workspace: null }
}
...
/**
 * A session that is no longer live, described by its last log record.
 *
...
function priceSession(ctx, session, depth, parentId, now, base) {
  ...
  const recordedTier = priced ? null : (typeof base?.tier === 'string' ? base.tier : null)
  return {
...
async function buildSummaryOnce(ctx, settings, state, sessionId, now = new Date()) {
  ...
  for (const id of tree.ids) {
    ...
    if (session === undefined) {
      // Not live: the log already priced whatever this session spent, so report
      // that rather than an empty row. A session the log never saw is reported
      // without a price, which is the honest answer.
      entries.push(recordedEntry(id, parentSessionId, depth, logged?.get(id)))
      continue
    }
