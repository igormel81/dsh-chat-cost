 ...
 * flush: overlapping runs would each see "no previous figure" and append the
 * same totals twice. One flush per tree at a time keeps the ledger a ledger.
 *
 * @param {object} ctx host plugin context
 * @param {object} settings resolved plugin config
 * @param {{ flushed: Map<string, number>, chains?: Map<string, Promise<unknown>> }} state per-plugin write memory
 * @param {string} sessionId root session id
 */
async function buildSummary(ctx, settings, state, sessionId) {
  const chains = state.chains ?? (state.chains = new Map())
  ...
  try {
    return await buildSummaryOnce(ctx, settings, state, sessionId)
  } finally {
    open()
    if (chains.get(sessionId) === queued) chains.delete(sessionId)
  }
}
