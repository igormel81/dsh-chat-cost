...
/**
 * How many provider calls this session made that its own usage does not contain.
 *
 * The web-search provider runs its searches inside one request to the
 * Anthropic-compatible endpoint with a server-side `web_search` tool: the model
 * fetches pages and reads them, and the provider bills that reading. The session
 * log keeps the request it sent and none of the usage that came back, so a
 * session figure is a floor and the invoice is higher. Counting those calls is
 * the one honest thing a local tool can do about it; inventing their tokens
 * would be the other, and a worse one.
 *
 * @param {Array<object>} events a live session's own event log
 * @returns {number} calls, zero when the log is not readable
 */
export function unbilledSearchCalls(events) {
  if (Array.isArray(events) === false) return 0
  let count = 0
  for (const event of events) {
    if (event?.type === SEARCH_CALL_EVENT) count += 1
  }
  return count
}
