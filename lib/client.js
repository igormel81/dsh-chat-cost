window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Retries while a finished turn waits for its numbers to reach the Host. */
		const TURN_RETRY_MS = 700;
		const TURN_RETRIES = 4;
		const LANGUAGES = ["en", "zh", "ru"];
		const FALLBACK_LANGUAGE = "en";

		/**
		 * One summary per session, shared by everything that displays it.
		 *
		 * The dock readout and the per-turn badge ask the Host for the same
		 * answer, so they share one poll instead of racing each other, and a
		 * badge can demand an immediate refresh the moment a turn ends — which is
		 * what makes the price appear with the answer rather than up to half a
		 * minute later.
		 */
		const stores = new Map();

		function summaryStore(sessionId) {
			let entry = stores.get(sessionId);
			if (entry === undefined) {
				entry = { sessionId, summary: null, failed: false, loaded: false, inflight: null, timer: null, subscribers: new Set() };
		...
		function publish(entry) {
			for (const listener of [...entry.subscribers]) {
				try { listener(); } catch { /* a broken subscriber must not stop the others */ }
			}
		}

		function loadSummary(entry, language) {
			if (entry.inflight !== null) return entry.inflight;
			entry.inflight = window.fetch(SUMMARY_PATH + "?sessionId=" + encodeURIComponent(entry.sessionId) + "&language=" + language, { headers: { accept: "application/json" } })
				.then((response) => (response.ok ? response.json() : Promise.reject(new Error(String(response.status)))))
				.then((body) => { entry.summary = body; entry.failed = false; })
				.catch(() => { entry.summary = null; entry.failed = true; })
				.then(() => { entry.inflight = null; entry.loaded = true; publish(entry); });
			return entry.inflight;
		}

		/** Subscribe to one session's summary, polling only while someone listens. */
		function useSummary(sessionId, language) {
			const entry = summaryStore(sessionId);
			const [, tick] = react.useState(0);
			react.useEffect(() => {
				const listener = () => tick((value) => value + 1);
				entry.subscribers.add(listener);
				if (entry.loaded === false && entry.inflight === null) loadSummary(entry, language);
				if (entry.timer === null) {
					entry.timer = window.setInterval(() => loadSummary(entry, language), REFRESH_MS);
				}
				return () => {
		...
		/** The Host's price for one turn, or null when it has none to give. */
		function turnOf(summary, turn) {
			if (summary === null || summary.ok !== true || Array.isArray(summary.turns) === false) return null;
			for (const entry of summary.turns) {
				if (entry !== null && typeof entry === "object" && entry.turn === turn) return entry;
			}
			return null;
		}
		...
		const STRINGS = {
			en: {
				...
				durable: (count, inherited) => count + " session(s) here are over: their counters come from the harness's durable record, not from a running session" + (inherited > 0 ? ", and " + inherited + " of them are priced by the route they inherited from the tree" : ""),
				searchCalls: (count) => count + " web-search LLM call(s) in this chat are billed by the provider but are not part of its token usage \u2014 the platform invoice is higher than this readout by their cost",
				recordedTip: "this chat is not open, so nothing here is watched live: the numbers come from the harness's durable record and this plugin's cost log",
				unknownTip: "the Host holds neither a live session nor a log record for this chat",
				version: (value) => "dsh-chat-cost " + value,
				turnTitle: "Cost of this answer",
				turnLine: (usd, model, tokens) => "\u2248 $" + usd + (model === null ? "" : " \u00b7 " + model) + " \u00b7 " + tokens + " tokens",
				turnUnpriced: "this turn ran on a model with no price in the catalog"
			},
			zh: {
				hostDown: "\u8d39\u7528\uff1a\u2014",
				...
				durable: (count, inherited) => "\u5176\u4e2d " + count + " \u4e2a\u4f1a\u8bdd\u5df2\u7ed3\u675f\uff1a\u5b83\u4eec\u7684\u8ba1\u6570\u6765\u81ea harness \u7684\u6301\u4e45\u8bb0\u5f55\uff0c\u800c\u975e\u6b63\u5728\u8fd0\u884c\u7684\u4f1a\u8bdd" + (inherited > 0 ? "\uff0c\u5176\u4e2d " + inherited + " \u4e2a\u6309\u4ece\u4f1a\u8bdd\u6811\u7ee7\u627f\u7684\u6a21\u578b\u8ba1\u4ef7" : ""),
				searchCalls: (count) => "\u672c\u5bf9\u8bdd\u6709 " + count + " \u6b21\u7f51\u7edc\u641c\u7d22 LLM \u8c03\u7528\u7531\u4f9b\u5e94\u5546\u8ba1\u8d39\uff0c\u4f46\u4e0d\u5728\u4f1a\u8bdd\u7684 token \u7528\u91cf\u4e4b\u5185 \u2014\u2014 \u5e73\u53f0\u8d26\u5355\u4f1a\u6bd4\u6b64\u8bfb\u6570\u591a\u51fa\u5b83\u4eec\u7684\u8d39\u7528",
				recordedTip: "\u8be5\u5bf9\u8bdd\u672a\u6253\u5f00\uff0c\u56e0\u6b64\u8fd9\u91cc\u6ca1\u6709\u5b9e\u65f6\u89c2\u6d4b\uff1a\u6570\u5b57\u6765\u81ea harness \u7684\u6301\u4e45\u8bb0\u5f55\u4e0e\u672c\u63d2\u4ef6\u7684\u8d39\u7528\u65e5\u5fd7",
				unknownTip: "\u5bbf\u4e3b\u65e2\u6ca1\u6709\u8be5\u5bf9\u8bdd\u7684\u6d3b\u52a8\u4f1a\u8bdd\uff0c\u4e5f\u6ca1\u6709\u5bf9\u5e94\u7684\u65e5\u5fd7\u8bb0\u5f55",
				version: (value) => "dsh-chat-cost " + value,
				turnTitle: "\u672c\u6b21\u56de\u7b54\u7684\u8d39\u7528",
				turnLine: (usd, model, tokens) => "\u2248 $" + usd + (model === null ? "" : " \u00b7 " + model) + " \u00b7 " + tokens + " tokens",
				turnUnpriced: "\u672c\u8f6e\u4f7f\u7528\u7684\u6a21\u578b\u5728\u4ef7\u683c\u76ee\u5f55\u4e2d\u6ca1\u6709\u4ef7\u683c"
			},
			ru: {
				hostDown: "\u0441\u0442\u043e\u0438\u043c\u043e\u0441\u0442\u044c: \u2014",
				...
				durable: (count, inherited) => count + " \u0441\u0435\u0441\u0441\u0438\u0439 \u0437\u0434\u0435\u0441\u044c \u0443\u0436\u0435 \u0437\u0430\u0432\u0435\u0440\u0448\u0435\u043d\u044b: \u0438\u0445 \u0441\u0447\u0451\u0442\u0447\u0438\u043a\u0438 \u0432\u0437\u044f\u0442\u044b \u0438\u0437 \u0434\u043e\u043b\u0433\u043e\u0432\u0435\u0447\u043d\u043e\u0439 \u0437\u0430\u043f\u0438\u0441\u0438 harness, \u0430 \u043d\u0435 \u0438\u0437 \u0440\u0430\u0431\u043e\u0442\u0430\u044e\u0449\u0435\u0439 \u0441\u0435\u0441\u0441\u0438\u0438" + (inherited > 0 ? ", \u0438 " + inherited + " \u0438\u0437 \u043d\u0438\u0445 \u043e\u0446\u0435\u043d\u0435\u043d\u044b \u043f\u043e \u043c\u0430\u0440\u0448\u0440\u0443\u0442\u0443, \u0443\u043d\u0430\u0441\u043b\u0435\u0434\u043e\u0432\u0430\u043d\u043d\u043e\u043c\u0443 \u043e\u0442 \u0434\u0435\u0440\u0435\u0432\u0430" : ""),
				searchCalls: (count) => "\u0432 \u044d\u0442\u043e\u043c \u0447\u0430\u0442\u0435 " + count + " \u0432\u044b\u0437\u043e\u0432(\u043e\u0432) LLM \u0434\u043b\u044f \u0432\u0435\u0431-\u043f\u043e\u0438\u0441\u043a\u0430: \u043f\u0440\u043e\u0432\u0430\u0439\u0434\u0435\u0440 \u0438\u0445 \u0442\u0430\u0440\u0438\u0444\u0438\u0446\u0438\u0440\u0443\u0435\u0442, \u043d\u043e \u0432 \u0440\u0430\u0441\u0445\u043e\u0434 \u0441\u0435\u0441\u0441\u0438\u0438 \u043e\u043d\u0438 \u043d\u0435 \u0432\u0445\u043e\u0434\u044f\u0442 \u2014 \u0441\u0447\u0451\u0442 \u043f\u043b\u0430\u0442\u0444\u043e\u0440\u043c\u044b \u0431\u0443\u0434\u0435\u0442 \u0432\u044b\u0448\u0435 \u044d\u0442\u043e\u0433\u043e \u043f\u043e\u043a\u0430\u0437\u0430\u043d\u0438\u044f \u043d\u0430 \u0438\u0445 \u0441\u0442\u043e\u0438\u043c\u043e\u0441\u0442\u044c",
				recordedTip: "\u0447\u0430\u0442 \u043d\u0435 \u043e\u0442\u043a\u0440\u044b\u0442, \u043f\u043e\u044d\u0442\u043e\u043c\u0443 \u0436\u0438\u0432\u043e\u0433\u043e \u043d\u0430\u0431\u043b\u044e\u0434\u0435\u043d\u0438\u044f \u0437\u0434\u0435\u0441\u044c \u043d\u0435\u0442: \u0446\u0438\u0444\u0440\u044b \u0432\u0437\u044f\u0442\u044b \u0438\u0437 \u0434\u043e\u043b\u0433\u043e\u0432\u0435\u0447\u043d\u043e\u0439 \u0437\u0430\u043f\u0438\u0441\u0438 harness \u0438 cost-\u043b\u043e\u0433\u0430 \u043f\u043b\u0430\u0433\u0438\u043d\u0430",
				unknownTip: "\u0443 \u0425\u043e\u0441\u0442\u0430 \u043d\u0435\u0442 \u043d\u0438 \u0436\u0438\u0432\u043e\u0439 \u0441\u0435\u0441\u0441\u0438\u0438, \u043d\u0438 \u0437\u0430\u043f\u0438\u0441\u0438 \u0432 \u043b\u043e\u0433\u0435 \u0434\u043b\u044f \u044d\u0442\u043e\u0433\u043e \u0447\u0430\u0442\u0430",
				version: (value) => "dsh-chat-cost " + value,
				turnTitle: "\u0421\u0442\u043e\u0438\u043c\u043e\u0441\u0442\u044c \u044d\u0442\u043e\u0433\u043e \u043e\u0442\u0432\u0435\u0442\u0430",
				turnLine: (usd, model, tokens) => "\u2248 $" + usd + (model === null ? "" : " \u00b7 " + model) + " \u00b7 " + tokens + " \u0442\u043e\u043a\u0435\u043d\u043e\u0432",
				turnUnpriced: "\u044d\u0442\u043e\u0442 \u0445\u043e\u0434 \u0448\u0451\u043b \u043d\u0430 \u043c\u043e\u0434\u0435\u043b\u0438, \u043a\u043e\u0442\u043e\u0440\u043e\u0439 \u043d\u0435\u0442 \u0432 \u043a\u0430\u0442\u0430\u043b\u043e\u0433\u0435 \u0446\u0435\u043d"
			}
		};
		...
		const apply = (ctx, config) => {
			...
			function ChatCost(props) {
				...
				// One subscription serves the readout and every turn badge on this
				// session, so a turn ending updates all of them at once.
				const entry = typeof props.sessionId === "string" ? useSummary(props.sessionId, langRef.current) : null;
				const summary = entry === null ? null : entry.summary;
				const failed = entry === null ? false : entry.failed;

				// The Host reports which language it answered in; that choice wins.
				...
				react.useEffect(() => {
					...
					loadSummary(entry, langRef.current);
					return undefined;
				}, [usageSignature, nonce, entry]);
				...
				if (summary !== null && summary.ok === true) {
					...
					// Sessions that ended are priced too, from the harness's own durable
					// record; the tooltip says which, so a catch-up never reads as a live
					// measurement.
					// Calls the provider bills and the session does not carry: the readout
					// says the figure is a floor rather than letting the invoice surprise.
					if (typeof summary.searchCalls === "number" && summary.searchCalls > 0) lines.push(text.searchCalls(summary.searchCalls));
					const durable = (summary.sessions || []).filter((entry) => entry.basis === "durable");
					if (durable.length > 0) lines.push(text.durable(durable.length, durable.filter((entry) => entry.modelSource === "inherited").length));
					...
					if (typeof summary.version === "string") lines.push(text.version(summary.version));
			...
			/**
			 * The price of one finished answer, rendered under it.
			 *
			 * The Host prices a turn from the session's own events, so the number
			 * is the turn's own spend rather than a slice of the session total. It
			 * may not have reached the Host at the instant the turn closes, so the
			 * badge asks again a few times before giving up — that is the whole
			 * difference between "instant" and "up to half a minute later".
			 */
			function TurnCost(props) {
				const matched = props !== null && typeof props === "object" ? props.matched : null;
				const turn = matched !== null && typeof matched === "object" && typeof matched.turn === "number"
					? matched.turn
					: (props !== null && props.turn !== null && typeof props.turn === "object" && typeof props.turn.turn === "number" ? props.turn.turn : null);
				const language = currentLanguage(null);
				const entry = useSummary(props.sessionId, language);
				const [attempt, setAttempt] = react.useState(0);
				const found = turn === null ? null : turnOf(entry.summary, turn);

				react.useEffect(() => {
					if (turn === null) return undefined;
					if (found !== null) return undefined;
					if (attempt >= TURN_RETRIES) return undefined;
					const timer = window.setTimeout(() => {
						setAttempt((value) => value + 1);
						loadSummary(entry, language);
					}, TURN_RETRY_MS);
					return () => window.clearTimeout(timer);
				}, [turn, attempt, found === null, entry]);

				if (turn === null || found === null) return null;
				const text = STRINGS[language] || STRINGS[FALLBACK_LANGUAGE];
				const shown = formatUsd(found.usd);
				if (shown === null) {
					return react.createElement("span", {
						className: "dsh-chat-cost dsh-chat-cost--muted",
				...
				const model = typeof found.model === "string" ? found.model : null;
				const tokens = typeof found.tokens === "number" ? found.tokens.toString() : "0";
				const lines = [text.turnTitle, text.turnLine(shown, model, tokens)];
				if (typeof found.pricingSource === "string" && found.pricingSource !== "none") {
					lines.push(found.pricingSource + (typeof found.tier === "string" && found.tier !== "flat" ? " \u00b7 " + found.tier : ""));
				}
				return react.createElement("span", {
					className: "dsh-chat-cost",
					title: lines.join("\n")
			...
			// Under every finished answer, not only in the composer: the cost of
			// that answer where the answer is.
			slots.inject("conversation.chat.turnTail", () => slots.register({
				name: "conversation.chat.turnTail",
				id: "dsh-chat-cost",
				order: 100,
				label: "Turn cost",
				select: (owner) => (owner !== null && owner !== undefined && owner.turn !== null && typeof owner.turn === "object" && typeof owner.turn.turn === "number"
					? { turn: owner.turn.turn }
					: null)
			}, (props) => react.createElement(TurnCost, props)));
		};
