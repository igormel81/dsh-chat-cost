...
import { buildScenarios, recommend } from './scenarios.js'
...
/** Route list of a unit spec, however the caller wrote it. */
const routesOf = (unit) => (Array.isArray(unit?.routes) ? unit.routes : (unit?.route ? [unit.route] : []))

/**
 * Identity of a plan's inputs. Scenario comparisons are only valid for the
 * specs they were priced from, so a refresh keeps them only when this matches.
 */
const specSignature = (units) => JSON.stringify((units ?? []).map((unit) => ({
  label: unit?.label ?? null,
  routes: routesOf(unit),
  requirements: unit?.requirements ?? null,
  critical: unit?.critical ?? unit?.criticality ?? null
})))
...
export function buildTools(deps) {
  const { catalog, settings, resolveProject, locks = null, languageOf = null } = deps

  /**
   * Plan and budget documents are read-modify-write, so every mutation goes
   * through one queue per project. Ledger appends are single writes and need no
   * lock; the summary's own per-tree queue is separate and independent.
   */
  const guarded = (key, work) => (locks === null ? work() : locks.run(key, work))
  const language = () => {
    if (typeof settings.language === 'string' && settings.language !== '') return settings.language
    const reported = typeof languageOf === 'function' ? languageOf() : null
    return typeof reported === 'string' && reported !== '' ? reported : 'en'
  }
    ...
    async execute(args) {
      ...
      for (const key of providers) {
        for (const id of Object.keys(catalog.providers[key]?.models ?? {})) {
          ...
          // Every field is emitted explicitly as a value or null: the registry
          // validates lossless JSON, and an absent key is not the same thing.
          matches.push({
            provider: key,
            model: id,
            name: catalog.providers[key].models[id]?.name ?? id,
            source: resolved.source,
            input: resolved.cost.input ?? null,
            cacheRead: resolved.cost.cacheRead ?? null,
            cacheWrite: resolved.cost.cacheWrite ?? null,
            output: resolved.cost.output ?? null,
            peakMultiplier: resolved.cost.peakMultiplier ?? 1
          })
  ...
  const plan = {
    name: 'cost_plan',
    description: 'Work with the project cost plan stored in <project>/.dsh-cost/: write or refresh it (priced and packed against the budget), read it back with actual spend merged, or set/clear/show the money budget. A refresh keeps the stored routing comparison when the plan inputs are unchanged, and drops it with a note when they moved. The plan file is meant to be edited by a human too.',
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
    ...
    async execute(args, exec) {
      ...
      if (action === 'budget') {
        if (args?.clear === true) {
          await guarded(`plan:${project.dir}`, () => writeBudget(project.dir, { usd: null, note: args?.note ?? null, clearedAt: new Date().toISOString() }, options))
        ...
        const path = await guarded(`plan:${project.dir}`, () => writeBudget(project.dir, { usd: args.budgetUsd, note: args?.note ?? null, setAt: new Date().toISOString() }, options))
      ...
      const document = {
        ...
        // The original specs (candidate routes, requirements, criticality) are
        // kept so cost_scenarios can re-price alternatives later; the packed
        // units above only carry the chosen route.
        unitSpecs: units,
        deferred: packed.deferred,
        totals: packed.totals,
        assumptions: { bufferRatio: 0.2, cacheReadShare: 0.3, uncertainty: 2 }
      }
      // Read-modify-write: the previous plan is read inside the same critical
      // section as the write, so a concurrent refresh or scenario pass cannot
      // land between them and lose its work.
      const written = await guarded(`plan:${project.dir}`, async () => {
        const previous = await readPlan(project.dir, options)
        const hadScenarios = Array.isArray(previous?.scenarios) && previous.scenarios.length > 0
        const sameInputs = previous !== null
          && Array.isArray(previous.unitSpecs) && previous.unitSpecs.length > 0
          && specSignature(previous.unitSpecs) === specSignature(units)
        const kept = { ...(sameInputs && hadScenarios ? { scenarios: previous.scenarios, recommendation: previous.recommendation } : {}) }
        const files = await writePlan(project.dir, { ...document, ...kept }, {
          dir: settings.logDirectory,
          context: { actuals, burn, language: language() }
        })
        return {
          files,
          plan: { ...document, ...kept },
          scenariosKept: sameInputs && hadScenarios,
          scenariosDropped: !sameInputs && hadScenarios
        }
      })
      return {
        ok: true,
        ...
        plan: written.plan,
        burn,
        scenariosKept: written.scenariosKept,
        scenariosDropped: written.scenariosDropped,
        ...(written.scenariosDropped
          ? { note: 'the stored routing comparison was priced for different units, so it was dropped; run cost_scenarios again' }
          : {})
      }
    }
  }
  ...
  const scenarios = {
    name: 'cost_scenarios',
    description: 'Compare routing scenarios for the project plan and tell the user which models are worth connecting: quality (the preferred route everywhere), connected (the cheapest route already listed), economy (the cheapest adequate model in the catalog, which may need a provider that is not connected yet) and balanced (preferred route for units marked critical, economy elsewhere). Adequacy uses catalog capability facts (reasoning, tools, vision, context); it is not a quality measurement. Results are saved into the plan file and shown in plan.md.',
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
        units: { type: 'array', description: 'Units to compare. Default: the units already in the plan file.', items: { type: 'object', additionalProperties: true } },
        budgetUsd: { type: 'number', description: 'Budget for the fit column. Default: the stored budget or the plan budget.' },
        includeFree: { type: 'boolean', description: 'Also price free-tier models, which are usually experimental or rate-limited. Default false.' },
        save: { type: 'boolean', description: 'Save the comparison into the plan file. Default true.' },
        custom: {
          type: 'array',
          description: 'Extra scenarios: a name plus a route list per unit label.',
    ...
    async execute(args, exec) {
      ...
      // The stored plan is read inside the lock: comparing scenarios and saving
      // them must not interleave with a concurrent plan refresh.
      return await guarded(`plan:${project.dir}`, async () => {
        const stored = await readPlan(project.dir, options)
        const storedBudget = await readBudget(project.dir, options)
        const specs = Array.isArray(stored?.unitSpecs) && stored.unitSpecs.length > 0
          ? stored.unitSpecs
          : (stored?.units ?? []).map((unit) => (routesOf(unit).length > 0 ? unit : { ...unit, routes: [] }))
        const units = Array.isArray(args?.units) && args.units.length > 0 ? args.units : specs
        if (units.length === 0) return { ok: false, reason: 'no-units', hint: 'write a plan first with cost_plan, or pass units' }
        const budgetUsd = numberOrNull(args?.budgetUsd) ?? numberOrNull(stored?.budgetUsd) ?? numberOrNull(storedBudget?.usd)

        const actuals = ledger === null ? [] : actualsByLabel(ledger.entries)
        const history = actuals.map((entry) => ({ label: entry.label, tokens: entry.tokens }))
        const built = buildScenarios({
          units,
          catalog,
          budgetUsd,
          history,
          includeFree: args?.includeFree === true,
          custom: Array.isArray(args?.custom) ? args.custom : []
        })
        const recommendation = recommend(built.scenarios, { budgetUsd, details: built })

        if (args?.save !== false && stored !== null) {
          const burn = ledger === null ? null : burnRate(spendSamples(ledger.costs), { budgetUsd, spentUsd: ledger.costs.reduce((sum, record) => sum + (typeof record.deltaUsd === 'number' ? record.deltaUsd : 0), 0) })
          await writePlan(project.dir, {
            ...stored,
            scenarios: built.scenarios.map((scenario) => ({
              name: scenario.name,
              description: scenario.description,
              totals: scenario.totals,
              providers: scenario.providers,
              routes: scenario.units.map((unit) => ({ label: unit.label, title: unit.title, route: unit.route, p50Usd: unit.p50Usd, p90Usd: unit.p90Usd }))
            })),
            recommendation: {
              savingsUsd: recommendation.savingsUsd,
              drivers: recommendation.drivers.slice(0, 5),
              opportunities: recommendation.opportunities.map((entry) => ({ label: entry.label, from: entry.from, to: entry.to, usd: entry.usd }))
            }
          }, { dir: settings.logDirectory, context: { actuals, burn, language: language() } })
        }

        return { ok: true, budgetUsd, ...built, recommendation }
      })
    }
  }

  return [price, history, estimate, plan, mark, scenarios]
}
