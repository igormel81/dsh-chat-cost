...
export function actualsByLabel(entriesOrCosts, legacyMarks = null) {
  ...
  for (const sessionId of sessions) {
    ...
    for (const entry of entries) {
      ...
      for (const key of Object.keys(bucket.tokens)) bucket.tokens[key] += Number(record.tokens?.[key] ?? 0)
...
export async function readLedger(projectDir, options = {}) {
  ...
  try {
    const parsed = parseLog(await readFile(path, 'utf8'))
    return { ...parsed, path }
  } catch {
    return { costs: [], marks: [], broken: [], path: null }
  }
}
