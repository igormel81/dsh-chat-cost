 ...
 * @param {'session'|'inherited'} [input.modelSource] where the route was learned
 * @param {'live'|'durable'|'log'} [input.basis] what the figure was read from
 ...
 *   provider?: string, model?: string, basis?: 'live'|'durable'|'log', modelSource?: 'session'|'inherited',
 *   usage: object, usd: number|null, pricingSource?: string}>} sessions
 */
export function summarizeTree(sessions) {
  const list = sessions.map((entry) => {
    ...
    return {
      ...
      modelSource: entry.modelSource ?? 'session',
      // Where the figure came from, per session: `live` from the running session,
      // `durable` from the harness's own checkpoint of a session that ended, `log`
      // from this plugin's ledger alone. The readout names it so a durable
      // catch-up is never mistaken for a watched one.
      basis: entry.basis ?? 'live',
